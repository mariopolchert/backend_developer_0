...
@section('content') 
    ...
    <h1>Uredi medij</h1>
    <hr>
    <form class="row g-3 mt-3" action="{{ route('formats.update', $format->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row mt-3">
            <div class="col-1">
        ...
        <div class="row mt-3">
            ...
            <div class="col-6">
                <input type="number" step="0.01" class="form-control" id="coefficient" name="coefficient" value="{{ $format->coefficient }}" required>
                @error('coefficient')
                    <span class="text-danger small">{{ $message }}</span>
                @enderror
            </div>
